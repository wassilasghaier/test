                               ...
                               <div class="iq-card">
                                 <div class="iq-card-header d-flex justify-content-center">
                                    <div class="iq-header-title">
                                       <h4 class="card-title">Modifier Club</h4>
                                    </div>
                                 </div>
                                 <div class="iq-card-body">
								   @foreach (['danger', 'warning', 'success', 'info'] as $message)
                                      ...
                                      @endif
                                   @endforeach
								         <form method="POST" action="{{ url('/admin/clubs/'.$club->id) }}" enctype="multipart/form-data">
                                   <input type="hidden" name="_method" value="PUT">
							                  {{ csrf_field() }}
                                       <div class="row justify-content-center">
                                          <div class="form-group col-sm-12 col-lg-12">
                                             <label for="fname">Nom club</label>
                                             <input type="text" class="form-control" name="name" id="fname" value = "{{$club->name}}" required = "required" onkeypress='return ((event.charCode >= 65 && event.charCode <= 90) || (event.charCode >= 97 && event.charCode <= 122) || (event.charCode == 32))'>
                                          ...
                                          <div class="form-group col-sm-12 col-lg-12">
                                             <label for="fname">Date de création Club</label>
                                             <input type="date" class="form-control" name="date_creation" id="fname" value="{{$club->date_creation}}" required = "required">
											            <span class="text-danger">@error('date_creation'){{ $message }} @enderror</span>
                                          </div>
                                          <div class="form-group col-sm-12 col-lg-12">
                                             <label for="fname">Date de debut des marches</label>
                                             <input type="date" class="form-control" name="date_debutmarhe" value="{{$club->date_debutmarhe}}" id="fname" required = "required">
											            <span class="text-danger">@error('date_debutmarhe'){{ $message }} @enderror</span>
                                          </div>
                                          <div class="form-group col-sm-12 col-lg-12">
                                             <label for="fname">Nombre des adhérents</label>
                                             <input type="number" class="form-control" name="nb_adherent" id="fname" placeholder="10" value="{{$club->nb_adherent}}" required = "required">
                                          ...
                                          <div class="row justify-content-center">
                                             ...
                                             @else
                                                <img src="{{url('/')}}/{{$club->photo}}" id="evt-img-tag" width="200px"/>
                                             @endif
                                          </div>
                                       </div>
                                       </div>
